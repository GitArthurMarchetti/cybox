...
export default function Departamento({ departamentos, departamento: novoDepartamento, userId }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Certifique-se de que o userId está presente e é um número
        if (!userId) {
            console.error('O ID do usuário não está disponível.');
            return;
        }

        const userIdNumber = Number(userId); // Converta o userId para número

        if (isNaN(userIdNumber)) {
            console.error('O ID do usuário não é um número válido.');
            return;
        }

        try {
            await saveDepartamento(formData, userIdNumber); // Passe o userId como número
        } catch (error) {
